...
const PassengerPage = () => {
    ...
    const handleRegister = async () => {
      ...
      const qrCodeLink = `https://aadhar-registration-abpsqvfyq-kavya-vs-projects.vercel.app/users/${aadhar}`;
...
<div className="=min-h-screen w-screen flex flex-col items-center justify-center bg-red-100 text-black px-4 space-y-6">
      ...
      {qrUrl && (
        <div>
          ...
          <button onClick={downloadImage} className="text-amber-50 ">Download as Image</button>
          <button onClick={downloadPDF} className="text-amber-50 ">Download as PDF</button>
        </div>
        </div>
      )}
      </div>
